...
| B站 CC 字幕直读 | 视频自带字幕时直接读取，跳过下载与识别，**秒级出稿**；AI 字幕需配置登录态 |
| Markdown 输出 | 带时间戳，便于对照原视频回查；可选各角色发言时长统计 |
| SRT 字幕 | 可与文稿一起导出，也可单独导出同名 `.srt`，直接拖进播放器当字幕；多人视频自动带角色前缀 |
...
| 视频链接 | 每行一个，可与上传文件混用 |
| 输出目录 | 默认 `output/`，可改成任意本地路径；文稿存到其中 `文稿/` 子文件夹、字幕存到 `srt/` 子文件夹；旁边有「打开输出目录」按钮 |
| 推理设备 | `auto` / `cuda:0` / `cpu` |
| 语言 | `auto` 或指定 `zh / yue / en / ja / ko`，指定后更准 |
| Markdown 样式 | `角色A：内容`（紧凑）或 角色名独立成行（长发言更清晰） |
| 导出 Markdown 文稿 / 导出 SRT 字幕 | 两个独立开关（默认都开），只想拿字幕就只勾 SRT；多位说话人时每条字幕带「角色A：」前缀，单人视频自动省略 |
...
| `--no-md` | 关闭 | 不导出 Markdown 文稿（只出 SRT 时用） |
| `--no-srt` | 关闭 | 不导出 SRT 字幕（默认同时导出） |
| `--srt-no-prefix` | 关闭 | SRT 字幕不加角色前缀 |
| `--no-cc` | 关闭 | B站链接不直读自带 CC 字幕，强制语音识别 |
...
│   ├── srt.py                SRT 字幕渲染（与 Markdown 共用切分与时间估算）
│   ├── ccsubs.py             B站 CC 字幕直读（view/player API + 字幕 JSON 解析）
...
**B站链接处理时日志里出现「CC 字幕探测失败」**
不影响使用，自动降级为语音识别。B站字幕分两种：UP 主手传的 CC 字幕与 AI 生成字幕，两者都要求**登录态**才能拉到列表。在 `local_config.py`（复制 `local_config.example.py` 改名）里配置 `BILIBILI_SESSDATA` 后重启即可直读，命中时秒级出稿、不下载不识别。

**输出目录可以自定义吗**
可以。GUI 顶部有输出目录输入框，CLI 用 `-o` 参数。默认写到 `output/`；无论输出到哪，文稿和字幕都会分开存（`文稿/` 与 `srt/` 两个子文件夹）。
